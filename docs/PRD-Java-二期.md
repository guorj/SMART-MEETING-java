...
> 日期：2026-05-29  
> 作者：阿统 🔧  
> 状态：二期能力图谱对齐「六阶段闭环」目标形态（§1.5）  
> 参考流程图：六阶段时间轴（会前24h → 会前10min → 会中60min → 会后5min → 会后两周 → 下次会议）；§2 保留三阶段业务流程总图
...
> **与 §1.5 关系**：§1.4 为产品叙事用的三阶段视图；§1.5 为二期目标态的**六阶段时间轴 + 实现进度**（与 [智能会议系统-项目最终形态-展示版.md](product/智能会议系统-项目最终形态-展示版.md) 对齐）。

### 1.5 二期能力图谱（六阶段 × 实现进度）

> **定位**：一张图讲清「要做什么」与「做到哪了」。  
> **与 §3 关系**：每项映射 F-PRE / F-MID / F-POST 编号；实现状态以代码库为准（2026-05-29）。  
> **图例**：🟢 已完成 · 🟡 进行中/部分可用 · ⬜ 未开始 · 🔶 产品目标与当前实现有偏差（见备注）
...
时间轴 ───────────────────────────────────────────────────────────────────→
        ...
        ┌────────┐    ┌────────┐    ┌──────────────┐    ┌────────┐    ┌────────┐    ┌────────┐
人的动作 │  无    │    │ 签到   │    │ 发言、决策   │    │   无   │    │点完成/ │    │   无   │
        │(组织者可│    │进入会议│    │(可手动加时)  │    │        │    │申请延期│    │        │
        │ 选延期)│    │        │    │              │    │        │    │        │    │        │
        └────────┘    └────────┘    └──────────────┘    └────────┘    └────────┘    └────────┘
        ┌────────┐    ┌────────┐    ┌──────────────┐    ┌────────┐    ┌────────┐    ┌────────┐
系统动作 │周期检测│    │会前盘点│    │AI主持+检点   │    │结束播报│    │每日催办│    │闭环通报│
        │自动筹备│    │签到卡片│    │议题切换+ASR  │    │纪要生成│    │临期弹窗│    │自动建会│
        │邀约确认│    │        │    │时间提醒      │    │待办提取│    │延期上报│    │        │
        │关键人  │    │        │    │              │    │飞书任务│    │        │    │        │
        │请假判断│    │        │    │              │    │        │    │        │    │        │
        │日历预约│    │        │    │              │    │        │    │        │    │        │
        └────────┘    └────────┘    └──────────────┘    └────────┘    └────────┘    └────────┘
进度     ⬜ 未开始      ⬜ 未开始      🟡 进行中         🟡 进行中      ⬜ 未开始      ⬜ 未开始
        (≈7%)         (≈10%)        (≈55%)            (≈50%)         (≈10%)         (≈15%)
```

**整体进度：≈ 36%**（会中能力最成熟；会前自动化与会后闭环跟踪仍为空白区；会后看板能力较完整）

#### 1.5.2 功能进度矩阵

| 阶段 | 能力点 | PRD 编号 | 人的动作 | 系统动作 | 状态 | 完成度 | 代码/模块依据 |
|------|--------|----------|----------|----------|------|--------|---------------|
| **会前24h** | 周期会议检测与自动筹备 | F-PRE-09 | 无 | 检测固定周期（如每周一 9:30 综合管理会），自动归集议题 | ⬜ | 0% | 无 `MeetingScheduler`；仅有 `int_meeting_type_preset` |
| | 飞书邀约卡片（参加/请假） | F-PRE-06 | 无 | 推送卡片；24h 不操作默认参加 | 🟡 | 10% | `ConfirmStatus` 枚举 + `Participant.status` 已建模；回调路由存在但仅打日志不写 DB |
| | **关键决策人请假 → 提示延期** | F-PRE-07 | 组织者确认是否延期 | 判断组织者/议题主导者/议题关联人 | ⬜ | 0% | PRD 有设计，代码未实现 |
| | 飞书日历 + 会议室预约 | F-PRE-04 | 无 | 创建日程、预约会议室、附上次纪要链接 | ⬜ | 0% | 无 `RoomBookingService` / 日历 API 对接 |
| | 议题归集（上周待办+新议题） | F-PRE-01/02 | 无 | 自动汇总议程 | 🟡 | 30% | `feishu-scheduled-bot` 会前事项对比（`weekly-comparison`）可部分替代 |
| **会前10min** | 会前盘点卡片 | F-PRE-10 | 签到、进入会议 | 展示签到/声纹/待办延期状态 | ⬜ | 10% | 混合检点后端有（`MeetingHostSessionService`），**缺会前10min 定时推送卡片** |
| | 声纹就绪检测 | F-PRE-05 延伸 | 无 | 未注册则提醒注册 | 🟡 | 60% | 声纹注册链路已通（ISV + admin 特征库） |
| **会中60min** | AI 自动开场 | F-MID-01 | 无 | TTS：「会议开始，议程 x 项…」 | 🟢 | 95% | `MeetingHostSessionService` 开场白 |
| | 会议检点点名 | F-MID-01 | 答到 | 线上盘点 + 线下 ASR 点名 | 🟢 | 90% | 混合检点完整实现 |
| | 上次待办进度通报（会中 TTS） | F-MID-02 | 责任人说明延期 | AI 播报「x 项完成，y 项延期…」 | 🟡 | 40% | 卡片 builder 有；**AI 主持开场未接入 previousMeetingId 待办统计** |
| | 议题切换 | F-MID-01 | 无 | 「现在进入议题一…」 | 🟢 | 95% | `nextTopic` / `skipTopic` |
| | 议题剩余 1 分钟提醒 | F-MID-09 | 无 | TTS/toast 提醒 | 🟢 | 100% | `topicWarnMinutes` tick |
| | **议题超时自动切换** | F-MID-01a | 决策是否加时 | 时间到自动下一议题 | 🔶 | 50% | **当前仅 toast 提示，需人工点「下一议题」**（见 `MeetingHostSessionService.tick` 注释） |
| | 会中决策/加时交互 | F-MID-01a | 点「议题加时」 | 延长当前议题计时 | 🟡 | 70% | 主持页 `extendTopic` 已实现；**议题内「决策记录」未建模** |
| | 实时 ASR + 声纹分离 | F-MID-04/06 | 发言 | 边录边转、标注说话人 | 🟡 | 60% | 实时 ASR ✅；声纹→姓名回写管线 **≈30%** |
| **会后5min** | 结束会议 + AI 播报 | F-MID-07 | 发起人点结束 | 「会议结束，纪要生成中」 | 🟢 | 90% | `MeetingService.endMeeting` + 飞书通知 |
| | 离线校正 → 声纹识别 → LLM 纪要 → 飞书文档 | F-MID-05/07 | 无 | 全自动流水线 | 🟡 | 65% | `MinuteGenerationService` ✅；**声纹识别写回 transcript 未闭环**；`OfflineCorrectionService` 为框架而非完整管线；低置信度 LLM 纠错 0%；VAD 选择性重识别未实现 |
| | 待办提取 + 责任人匹配 | F-POST-01 | 无 | LLM 拆解 + 姓名→user_id | 🟡 | 85% | `TodoExtractionService` |
| | 创建飞书任务 + 群推送 | F-POST-01 | 无 | 同步到飞书任务系统 | 🟡 | 25% | `FeishuTaskService` **仅发文本消息**，未对接飞书 Task API |
| | 待办状态看板 | F-POST-05 | 查看看板 | 会议+人员维度双视图 | 🟢 | 90% | `TodoBoardResponse` API 完整，`TodoController` 提供会议/人员维度查询 |
| | 手动修正（重命名说话人/重新生成纪要） | F-POST-06 | 发指令 | `rename_speaker` / `regenerate_minutes` | ⬜ | 0% | `FeishuCommandRouter` 有路由但两个指令均为 TODO stub |
| **会后两周** | 每日待办提醒 | F-POST-02 | 无 | 飞书消息催办 | 🟡 | 20% | `TodoReminderScheduler` 有骨架，**缺 24h 前弹窗卡片 + 完成/延期 action** |
| | 截止前 24h「完成/申请延期」 | F-POST-03 | 点卡片 | 推送确认；延期转上级 | 🟡 | 15% | 卡片路由/callback 基础设施存在（`FeishuWebhookController` L630-634），但仅打日志不写 DB |
| | 超期自动 DELAYED + 上级通知 | F-POST-03 | 无 | 自动标记 + 升级 | ⬜ | 0% | PRD 有 `autoMarkDelayed`，代码未落地 |
| | 自动创建下次会议邀约 | F-POST-04 | 无 | 附上次纪要 + 待办统计 | ⬜ | 0% | `previousMeetingId` 字段有，**无自动建会调度** |
| **下次会议** | AI 开场闭环通报 | F-POST-04 / F-MID-02 | 无 | 「上次 3 项待办：2 完成 1 延期…」 | 🟡 | 40% | 数据模型 + 卡片 builder 有；**与 AI 主持 TTS 未串联** |
| | 闭环重启（议题含未完成任务） | F-POST-04 | 无 | 新会议 agenda 带入 pending todos | ⬜ | 10% | 仅手动 `previousMeetingId` 关联 |

#### 1.5.3 阶段进度汇总

| 阶段 | 阶段进度 | 说明 |
|------|----------|------|
| 会前24h | **≈ 7%** | 周期自动发起、参会确认、关键人判断、日历预约均未落地；weekly-comparison 为局部替代；ConfirmStatus 数据模型已建 |
| 会前10min | **≈ 10%** | 检点能力在会中可用，缺「会前10min 定时盘点卡片」 |
| 会中60min | **≈ 55%** | AI 主持 + 检点 + 议题计时为当前最强项；声纹闭环与会中待办通报待补 |
| 会后5min | **≈ 50%** | 纪要生成主链路可用；待办看板 90%；飞书任务 API、声纹写回为缺口 |
| 会后两周 | **≈ 10%** | 提醒/延期/升级/自动建会基本空白 |
| 下次会议 | **≈ 15%** | 数据关联已有，自动化闭环未通 |

#### 1.5.4 与 §3 功能清单的映射说明

| 原 §3 组织方式 | 本图谱组织方式 | 变更要点 |
|----------------|----------------|----------|
| 三阶段（会前/会中/会后） | 六时间点（+ 会后两周 + 下次会议） | 突出「用户零操作」与「系统全自动化」的时间感 |
| 功能平铺表格 | 功能 × 进度 × 代码依据 | 每项可验收、可排期 |
| F-PRE-07 关键人回流 | 会前24h 独立高亮 | 与目标形态「关键决策人请假→提示延期」对齐 |
| F-MID-02 上次通报 | 拆为「会中 TTS 通报」+「下次会议闭环」 | 避免与会前卡片（已 v0.9 下线）混淆 |

**v0.9 架构变更（须在图谱中显式标注）**

- 会前「上次待办进度卡片」已由 **feishu-scheduled-bot 会前事项对比**（`int_weekly_matter_comparison_job`）部分取代，不再由 meeting-server 建会时推送。
- 图谱中「议题归集」与「上次纪要附带」应标注 **Bot 侧能力**，避免与 meeting-server 会前模块重复计数。

#### 1.5.5 二期补齐路径（按图谱空白区）

| 优先级 | 目标能力 | 主要交付 | 预估 |
|--------|----------|----------|------|
| **P0** | 会前24h 自动筹备 | `MeetingScheduler` + 参会确认卡片 + 状态机 + 24h 超时默认 | 1–2 周 |
| **P0** | 关键决策人请假→延期提示 | 关键人规则引擎 + 组织者确认卡片 | 3–5 天 |
| **P0** | 声纹→transcript 闭环 | 会后 ISV 搜索 + 写回 + 纪要人名 | 1 周 |
| **P1** | 议题超时策略可配置 | 「仅提醒 / 自动切换 / 等待决策」三模式（F-MID-01a） | 2–3 天 |
| **P1** | 飞书 Task API 真同步 | 替换 `FeishuTaskService` 文本 stub | 1 周 |
| **P1** | 会后两周跟踪 | 24h 卡片 + complete/delay action + DELAYED 扫描 | 1–2 周 |
| **P2** | 下次会议自动创建 | `previousMeetingId` 链 + 周期调度 + agenda 带入 pending | 1 周 |
| **P2** | 会前10min 盘点卡片 | 定时任务 + 签到/声纹/待办状态聚合 | 3–5 天 |

#### 1.5.6 验收口径（图谱变绿的条件）

1. **会前24h 变 🟢**：固定周期会议无需人工「开始会议」，参会确认率可统计，关键人请假触发组织者决策流。
2. **会中60min 变 🟢**：transcript 中 ≥70% 发言段落到真实姓名；AI 开场含上次待办统计；议题超时策略与产品一致。
3. **会后5min 变 🟢**：纪要 5–10 分钟内出文档；待办写入 DB **且** 同步飞书 Task。
4. **会后两周 + 下次会议变 🟢**：临期/超期/延期审批全自动；下次周期会议自动创建并 TTS 闭环通报。

#### 1.5.7 基础设施维度（代码验证: 2026-05-29）

| 维度 | 完成度 | 关键依据 | 最大缺口 |
|------|--------|---------|---------|
| 消息队列（Kafka/LocalEventBus） | 70% | `KafkaProducer`+`MinuteGenerateConsumer`+`TodoExtractConsumer`+4 topic+DLT+Outbox 完整编码；默认 LocalEventBus | 切 Kafka 需改配置+启动容器；docker-compose 已含 Kafka 3.7.0 |
| 状态机 | 90% | Spring StateMachine 15 状态+14 事件+20+ 转换+DB 恢复+forceStatus | 缺 `ATTENDEE_CONFIRMING`/`AGENDA_SENT`；`REVIEWING` 无转换 |
| Pipeline 编排 | 60% | `PipelineStepDispatcher`+`TimeoutScanner`+3 表+Admin CRUD+UI 完整 | 2/4 executor 为 placeholder/stub；纪要/待办/ASR 走 MQ 直链 |
| 管理后台 | 85% | 10 模块注册+动态加载 SPA shell+各模块 JS | 个别模块按钮可用性待优化 |
| 飞书集成 | 90% | `FeishuService` 909 行+`WebhookController` 717 行+卡片+文档+通讯录 | 缺日历 API/Task API |
| 声纹生命周期 | 85% | `XfyunIsvClient` 548 行+注册/删除/搜索/缓存/回滚 | 管线未闭环（→transcript 写回） |
| Auth/安全 | 10% | 3 层 token interceptor（admin/internal/API-key）+JWT 多类型 | 无 Spring Security/RBAC/HTTPS 强制 |
| 数据库 | 95% | PRD 6 表全在+10 扩展表+6 bot 表+v0.4–v0.20 升级脚本 | meeting-server 无 Flyway（手动迁移） |
...
## 3. 功能需求（按流程图三阶段组织，图谱阶段见 §1.5）

> 各功能项「图谱阶段」列与 **§1.5.2 功能进度矩阵** 对齐，便于排期与验收。
...
| 编号 | 功能 | 描述 | 优先级 | 图谱阶段 | 流程图对应节点 |
|------|------|------|--------|----------|---------------|
| F-PRE-01 | 收集议题与参会人拟定 | 发起人预约（可临时预约、定时会议则由系统到时自动发起）会议（会议主题+参会人【此时应有列表显示参会人在时间段空闲状态】+期待开始时间）请求，系统向相应参会人推送会议预约信息，同时接收参会人反馈：是否参会（会议开始前24确认，超时不确认默认参会；2h<当前时间与会议时间间隔<24h,则需会议开始前2h确认；2h>当前时间与会议时间间隔>30min，则需在30分钟内确认；当前时间与会议时间间隔<30min,则发送紧急通知，需要收到后5分钟内确认），参会议题（可不填），预计时长（默认5分钟），是否新增参会人（默认不新增） | P0 | 会前24h | 收集议题、拟定参会人 
| F-PRE-02 | 议题汇总与参会人确认 | 系统汇总议题确认议程、确认新增参会人是否参会 | P0 | 会前24h | 确定参会人 |
| F-PRE-03 | 发送议程 | 向所有参会人推送会议议程（议题列表+讨论框架+预计时长，无论参会人是否参会），参会人可提前预览准备材料 | P0 | 会前24h | 发送议程 |
| F-PRE-04 | 会议室预约 | 对接飞书日历（自动创建日程）/会议室API（二期），自动查询空闲时段并预约；支持线上会议创建（飞书视频会议） | P1 | 会前24h | 会议室预约 |
| F-PRE-05 | 会议资料推送 | 推送会议背景资料：上次会议纪要（含待办完成情况）、相关业务数据、参考文档等；资料以飞书消息附件/链接形式推送 | P1 | 会前24h | 会议资料推送 |
| F-PRE-06 | 参会确认 | 向参会人发送确认请求（飞书卡片消息，一键"参加/请假"），系统统计确认结果并展示确认率 | P0 | 会前24h | 参会确认（判断节点） |
| F-PRE-07 | 参会人调整 | 当有人确认"不参加"时（尤其是关键决策者），系统提示发起人调整参会人名单或二次邀约；支持设置"必须参加"标记 | P0 | 会前24h | 否→回流至"确定参会人" |
| F-PRE-08 | 会议邀约 | 确认完成后发送正式邀约（飞书日程邀请），含时间、地点/线上链接、议程、资料包；同时生成录音页面链接供会中使用 | P0 | 会前24h | 会议邀约 |
| F-PRE-09 | 周期会议自动触发 | 按会议组/预设 Cron 自动创建下次会议实例，归集上周待办与群内新议题；无需人工发送「开始会议」 | P0 | 会前24h | 收集议题（自动化入口） |
| F-PRE-10 | 会前10min 盘点卡片 | 会议开始前 10 分钟推送盘点卡片：签到状态、声纹就绪、上次待办延期项；一键进入会议/录音 | P1 | 会前10min | —（见 [AI会议主持人PRD_v1.md](product/AI会议主持人PRD_v1.md) US-05） |
...
> **流程入口**：会议邀约时间到达，用户发送"会议管理"进入 dashboard 工作台后发起会议，或点击邀约卡片按钮。
> **流程出口**：纪要生成完成 → 进入会后跟踪阶段。

| 编号 | 功能 | 描述 | 优先级 | 图谱阶段 | 流程图对应节点 |
|------|------|------|--------|----------|---------------|
| F-MID-01 | 会议开始 | 承接邀约正式开启会议，系统自动进入记录就绪状态；若关联上次会议，自动拉取待办统计数据 | P0 | 会中60min | 会议开始 |
| F-MID-01a | 议题超时策略 | 议题计时到时后的行为：**仅提醒**（当前默认）/ **自动切换下一议题** / **等待人工决策**；与主持页「议题加时」联动 | P1 | 会中60min | 会议开始 |
| F-MID-02 | 上次会议进度通报 | **【新增-流程图关键节点】** 会议开始时，自动展示上次会议待办事项的完成情况统计（已完成X/未完成Y/进行中Z），对延期项高亮标注并请责任人说明原因，保障事项跟进连续性 | P1 | 会中60min / 下次会议 | 上次会议进度通报 |
| F-MID-03 | 会议录音启动 | 用户在 dashboard 创建会议后，点击邀约卡片中的录音按钮，系统启动Web录音页面（需HTTPS+麦克风权限） | P0 | 会中60min | 会议录音 |
| F-MID-04 | 实时录音解析 | 边录边转文字：浏览器通过AudioContext采样16kHz单声道PCM，WebSocket每40ms推送一帧（1280字节）至后端ASR引擎，Web页面实时回显转写结果（初稿） | P0 | 会中60min | 实时录音解析 |
| F-MID-05 | 离线录音校正 | 录音结束后对音频文件二次校验：①修正口音/术语识别误差 ②对低置信度片段（confidence<0.7）用大模型做文本级纠错 ③VAD检测信号质量差的片段从本地音频缓存重新识别 | P1 | 会后5min | 离线录音校正 |
| F-MID-06 | 发言人识别 | 基于音色识别大模型（讯飞ISV声纹），通过语音特征区分不同发言人，将speaker_0/1/2映射为实际参会人姓名 | P0 | 会中60min / 会后5min | 发言人识别 |
| F-MID-07 | 生成文字版会议纪要 | 整合实时解析+离线校正+发言人信息，大模型按标准模板生成结构化纪要（议题回顾→讨论要点→决议→待办），写入飞书文档 | P0 | 会后5min | 生成文字版会议纪要 |
| F-MID-08 | 暂停/继续录音 | 支持茶歇时段暂停录音（暂停推流但保持WebSocket连接），节省ASR配额；恢复后自动续传 | P2 | 会中60min | — |
| F-MID-09 | 超时保护 | 最大录音时长4h（超时自动停止并通知参会人）；连续30min无有效语音（VAD静音检测）自动暂停并提醒；每小时推送录音状态提醒（时长/费用预估）；议题剩余1分钟 TTS 提醒 | P0 | 会中60min | — |
...
| 编号 | 功能 | 描述 | 优先级 | 图谱阶段 | 流程图对应节点 |
|------|------|------|--------|----------|---------------|
| F-POST-01 | 同步到责任人待办 | 从纪要中拆解待办任务，分配给对应责任人（通过user_id），明确任务内容、要求与截止时间；同步到飞书待办/任务系统；若声纹已注册则自动匹配人名。<br>**责任人匹配逻辑**：优先通过 `int_meeting_participant.name` → `userId` 匹配；多人同名 → 标注"待确认"，通知发起人手动指定；无法匹配 → `assigneeId` 暂空，记录到"未分配"列表。 | P0 | 会后5min | 同步到责任人待办 |
| F-POST-02 | 进度提醒 | 向责任人发送进度提醒：临近截止通知（截止前24h / 2h）、阶段性节点提醒（如按周迭代）；可配置提醒频率和免打扰时段 | P1 | 会后两周 | 进度提醒 |
| F-POST-03 | 进度跟踪 | 持续跟踪待办执行进度：责任人可通过飞书卡片"标记完成"/"申请延期"更新状态；发起人可查看所有待办状态看板（按会议按人统计）；记录完成情况与卡点问题 | P1 | 会后两周 | 进度跟踪 |
| F-POST-04 | 回流至下次会议 | **【闭环关键】** 下次会议开场时，通过 previousMeetingId 自动关联，统计并通报上次会议待办完成情况（已完成/进行中/未开始/已延期），形成持续跟进循环 | P1 | 下次会议 | 回流至「开始」 |
| F-POST-05 | 待办状态看板 | 提供会议维度+人员维度双视图待办看板：本次会议全部待办、状态分布饼图、延期项红色高亮标记 | P2 | 会后两周 | — |
| F-POST-06 | 手动修正 | 用户可在飞书文档中手动修正说话人标记错误、待办分配错误，触发纪要重新生成（不重新录音，仅重跑大模型+文档替换） | P2 | 会后5min | — |
...
### 4.5 Pipeline 复用度评估结论（v2.2 增量实现）

| 结论项 | 当前结果 | 落地说明 |
|--------|----------|----------|
| 基础设施复用率 | **约 85%** | `PipelineStepDispatcher`、`PipelineTimeoutScanner`、执行记录表与 Admin CRUD 全量复用 |
| 二期能力配置化覆盖 | **约 77%** | 通过新增 StepExecutor + `configJson` 可覆盖多数会前/会后编排能力 |
| 非 Pipeline 能力占比 | **约 23%** | 包括定时触发器（MeetingScheduler）、用户即时指令路由、复杂异步事件回调链 |
| 异步回调能力 | **已补齐** | 新增 `WAITING_CALLBACK` 状态 + `PipelineCallbackRouter`，支持「推卡片 → 等回调 → 续跑」 |
| 条件分支与跨步骤上下文 | **已补齐基础能力** | `configJson.condition` + `sharedContextJson`，可实现基础 if/else 与上下文透传 |

> 说明：本版本将低复杂度与中复杂度 Executor 批量落地，并补齐高复杂度闭环入口（`post-voiceprint-identify`、`post-offline-llm-correction`）。剩余复杂业务策略（如多分支审批链）在后续版本继续增强。
...
> 分期与 **§1.5 二期能力图谱** 对齐；历史 P0-MVP～P3 计划保留于 §12.2 供回溯。

### 12.1 分期交付（按六阶段图谱，2026-05-29 起）

| 阶段 | 时间 | 范围（§1.5 空白区） | 交付物 | 图谱目标 |
|------|------|---------------------|--------|----------|
| **G0-已交付** | — | 会中核心 + 纪要 + AI 主持骨架 | 录音/ASR/纪要/混合检点/主持 TTS/待办提取 | 会中 ≈55%、会后5min ≈45% |
| **G1-会前** | 第1–2周 | F-PRE-09/06/07/04/08 | `MeetingScheduler`、参会确认状态机、关键人延期卡片、飞书日历 | 会前24h → ≈60% |
| **G2-声纹闭环** | 第3周 | F-MID-06 + 声纹写回 | ISV 会后识别→transcript 写回→纪要人名 | 会中/会后5min +15% |
| **G3-会后跟踪** | 第4–5周 | F-POST-01/02/03 | 飞书 Task API、24h 卡片、DELAYED 扫描、延期审批 | 会后两周 → ≈70% |
| **G4-闭环** | 第6周 | F-POST-04 + F-PRE-10 + F-MID-02 | 周期自动建会、会前10min 盘点、AI TTS 闭环通报 | 下次会议 → ≈80%；**整体 ≈70%** |
...
| 维度 | v1.1-java | v2.0-java | v2.1-java | v2.2-java（本版） |
|------|-----------|-----------|-----------|-------------------|
| **用户标识** | open_id（应用内唯一） | user_id（企业内唯一），辅以 int_user_mapping_feishu 表建立OA↔飞书双射 | 不变 | 不变 |
| **表名前缀** | meeting / meeting_participant / meeting_todo | int_meeting / int_meeting_participant / int_meeting_todo | 不变 | 不变 |
| **meeting表结构** | 无company/department/group字段 | 新增company(集团)、department(部门)、groupName(会议组) | 不变 | 不变 |
| **voiceprint表** | 以open_id为主键关联用户 | 以userId(int)关联OA系统用户，feishuUserId通过mapping映射获取 | 新增feishu_user_id索引 | 不变 |
| **新增表** | — | int_user_mapping_feishu（OA用户ID↔飞书三种ID映射） | 不变 | 不变 |
| **消息队列** | RabbitMQ（延时队列+死信） | Kafka（高吞吐+分区有序+DLQ Topic） | todo.extract独立Topic；todo.reminders→Redis ZSet | 不变 |
| **声纹注册API** | 参数为openId | 参数为userId(int) | 不变 | 不变 |
| **参与人/待办责任人** | openId | userId（飞书user_id） | 不变 | 不变 |
| **配置更新** | 本地开发环境占位符 | 真实数据库连接 + 每日8:30定时提醒 + 延期次数可配 | 不变 | 不变 |
| **枚举定义** | 仅MeetingStatus | 补全TODO_TRACKING状态 | 新增ConfirmStatus/TodoStatus/Priority | 不变 |
| **风险项** | 7项 | 新增"OA↔飞书ID映射缺失"风险 | 新增"WebSocket断线导致音频丢失" | 不变 |
| **能力图谱** | — | 三阶段流程图 | 15项查缺补漏 | **新增 §1.5 六阶段时间轴 + 实现进度矩阵（整体 ≈35%）** |
| **功能编号** | — | F-PRE/MID/POST | 不变 | 新增 F-PRE-09/10、F-MID-01a；§3 增加「图谱阶段」列 |
| **交付计划** | — | P0–P3 18天 | 不变 | §12.1 按 G0–G4 与图谱对齐；原 P0–P3 移至 §12.2 |
