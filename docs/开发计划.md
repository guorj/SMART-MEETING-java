...
| 会中体验 | 会中群静音时「会议管理」改 **单聊** 发卡，避免无弹窗 |
...
| 飞书入口 | `FeishuCommandHandler.handleOpenDashboard` | 权限校验；群 mute 时 **单聊** 发卡/拒绝 |
...
- **限制**：同一飞书群 `chat_id` 同时仅建议一场（`MeetingHostFeishuMuteRegistry` 按群静音）
...
- 集成：白名单拒绝 / 部分权限 / 会中单聊发卡 / 主控+旁观 WS / 2 创建人并行建会
...
| 会中群 mute 无卡片 | `handleOpenDashboard` 单聊兜底 |
...
- [x] `DashboardController` / `FeishuCommandHandler` 权限与单聊兜底
