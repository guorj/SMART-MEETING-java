...
| 3 | 往期 to-do 审议 | 开发中 | 50% | **50%** | `getPreviousProgress` API；会前 `progress-analysis` Skill | **会前**飞书卡片；会议主页会序通报走 `AgendaBriefingService`（另项） |
| 4 | 会议序列自动切换 | 已完成 | 100% | **85%** | `nextTopic` / `skipTopic` + TTS | 依赖人工点按钮；检点类会序结束可自动下一议题 |
| 5 | 文件随会序切换 | 开发中 | 20% | **25%** | `mergePresetAgendaDocs` + `GET /agenda-doc-content`；会议主页 `syncAgendaDocPanel` | **会议主页未调用** agenda-doc-content，仅展示外链 |
...
| 功能 | 达到 80% 还差什么 |
|------|-------------------|
| 会序 OpenClaw 通报 | `openclawBriefing` 会序进入 RUNNING → `AgendaBriefingService` → 主持区 Markdown（TTS 摘要播报待产品） |
...
- [x] 会序 `openclawBriefing=true` 进入 RUNNING：`AgendaBriefingService` + `matter-progress` Skill → 主持区 `briefingMarkdown`（旧录音页 API `matter-progress-report` 已移除）
- [ ] `AgendaBriefingService`：OpenClaw 超时/失败降级文案；通报 TTS 摘要播报（`speakAsync`）待产品
...
- [ ] `int_matter_progress_doc_config` 按预设 + `agenda_index` 配置飞书 URL；通报行设 `openclaw_briefing=1`（默认 0）
...
| 05-29 | 会序 OpenClaw 通报 | 进入 `openclawBriefing` 会序后主持区自动生成 Markdown（非旧版独立录音页） |
...
| 周 | 开发侧 | 你方 |
|----|--------|------|
| W1 | 会议主页接 agenda-doc-content；会序通报 TTS 摘要（可选） | 配 `int_matter_progress_doc_config`（URL + `openclaw_briefing`）或 `host_agenda.openclawBriefing` |
...
| 会序 OpenClaw 通报 | `meeting-server/src/main/java/com/smartmeeting/service/host/AgendaBriefingService.java` |
