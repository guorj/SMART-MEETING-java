...
| 4 | HikariCP `maximum-pool-size: 40`（临时） | `application.yml` / `application-prod.yml` | 缓解池耗尽 |

### P1 — 功能完整多会议并行（约 3–5 人日）

| # | 改动 | 预期效果 |
| - | ---- | -------- |
| 5 | Xfyun ASR 按 meetingId 连接池 | 多路实时转写 |
| 6 | `meetingTaskExecutor` 扩容 + 可配置 | 会后任务并行 |
| 7 | OpenClaw WS 连接池或长连接复用 | 降低握手开销 |
| 8 | LLM/Feishu 简单重试 + 超时 | 提高成功率 |
...
| `spring.datasource.hikari.maximum-pool-size` | 20 | 并发 DB 上限 |
