...
| `recording_url` | VARCHAR(500) | — | — | 录音页面 URL（浏览器录音前端） |
| `recording_token` | VARCHAR(500) | — | — | 录音页面 JWT token |
...
**用途**：存储 meeting-server 运行时的可热改参数（如主持流程超时时间、TTS 语速等），无需重启即可生效。

**使用场景**：
- meeting-admin-server 提供管理界面修改
- meeting-server 通过 `InternalReloadProperties` 热加载（或定时轮询）
