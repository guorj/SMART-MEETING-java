...
public class JdbcPresetHostAgendaConfigRepository implements MatterProgressConfigRepository {
    ...
    public Optional<MatterProgressConfigRow> findByConfigName(String configName) {
        ...
        for (int code = 1; code <= 5; code++) {
            ...
            Optional<HostAgendaJsonCodec.LocatedDoc> located =
                    HostAgendaJsonCodec.findDocByConfigName(json, name, mapper);
        ...
        for (String n : configNames) {
            findByConfigName(n).ifPresent(row -> {
                if (row.enabled() && row.feishuDocUrl() != null && !row.feishuDocUrl().isBlank()) {
                    ...
                    if ("SOURCE".equals(role) || "BOTH".equals(role)) {
                        out.add(row);
                    }
                }
            });
    ...
    private static MatterProgressConfigRow toRow(int presetCode, HostAgendaJsonCodec.LocatedDoc located) {
