 ...
 * 
 * 流程:
 * 1. POST /v2/upload → 上传二进制音频 → 获取 orderId
 * 2. POST /v2/getResult → 轮询结果 → 获取转写文本
 *
 * 关键差异（与旧版API）：
 * - BASE_URL: office-api-ist-dx.iflyaisol.com
 * - Content-Type: application/octet-stream（二进制流）
 * - 参数: URL查询参数（不编码！讯飞不接受编码后的dateTime）
 * - 签名: TreeMap字典序 + URL编码 + HMAC-SHA1
 */
@Slf4j
@Component
...
public class XfyunOfflineClient {
     ...
     * 离线转写音频数据
     *
     * @param audioData 音频数据（PCM格式，16kHz，16bit，单声道）
     * @param filename 文件名
     * @return 转写结果列表（带时间戳分段）
     */
    ...
    /**
     * 生成时间格式（yyyy-MM-dd'T'HH:mm:ss+0800）
     * 文档要求：RFC 822 时区格式，如 2025-09-08T22:58:29+0800
     */
    ...
    /**
     * 估算音频时长（PCM: 16kHz, 16bit, 单声道）
     * 公式：字节数 / (16000 * 2) * 1000 = 毫秒
     */
     ...
     * 2. TreeMap字典序排序
     * 3. 键值URL编码（签名计算需要编码）
     * 4. 构建 baseString: key1=value1&key2=value2
     * 5. HMAC-SHA1加密后Base64编码
     */
