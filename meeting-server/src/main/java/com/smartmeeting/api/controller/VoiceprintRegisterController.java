...
public class VoiceprintRegisterController {
    ...
    private String buildRegisterPage(String token, String userName) {
        ...
        html.append("<p style=\"text-align:center;color:#999;font-size:12px;margin-top:20px;\">建议录音不少于 35 秒，注册链接 30 分钟内有效</p>\n");
        html.append("</div>\n");
        html.append("<script>\n");
        html.append("let mediaRecorder=null,audioChunks=[],startTime=null,timerInterval=null;\n");
        html.append("const token='" + token + "',MIN_DURATION=35;\n");
        html.append("const API_BASE=location.pathname.replace(/\\/voiceprint$/,'');\n");
        ...
        html.append("try{\n");
        html.append("const res=await fetch(API_BASE+'/api/v1/voiceprint/register',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token,audio:base64})});\n");
